...
class InstallationTests(unittest.TestCase):
    ...
    def test_a_read_only_install_directory_is_treated_as_installed(self) -> None:
        """Without this the updater would fetch a zip it cannot unpack.

        Writability is patched rather than simulated: this must hold on Windows,
        where POSIX mode bits do not make a directory read-only.
        """
        from unittest import mock

        from elite_hud.installation import INSTALLED, _fallback_install_info

        with mock.patch("elite_hud.installation.is_writable_dir", return_value=False):
            self.assertEqual(_fallback_install_info().mode, INSTALLED)

    def test_a_writable_install_directory_is_treated_as_portable(self) -> None:
        from unittest import mock

        from elite_hud.installation import PORTABLE, _fallback_install_info

        with mock.patch("elite_hud.installation.is_writable_dir", return_value=True):
            self.assertEqual(_fallback_install_info().mode, PORTABLE)
